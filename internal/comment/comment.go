...
var (
	ErrFetchingAllComments = errors.New("failed to fetch all comments")
...
// Service - the struct on which
// all our logic will be built on top of
type Service struct {
	Store Store
}
...
func (s *Service) GetComments(ctx context.Context) ([]Comment, error) {
	...
	if err != nil {
		fmt.Println(err)
		return nil, ErrFetchingAllComments
	}
	return cmts, nil
}
